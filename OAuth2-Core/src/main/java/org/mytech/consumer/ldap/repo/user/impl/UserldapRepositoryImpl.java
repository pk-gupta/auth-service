package org.mytech.consumer.ldap.repo.user.impl;
...
import org.mytech.consumer.ldap.repo.user.LoginRepository;
import org.mytech.consumer.ldap.repo.user.PersonRepository;
import org.mytech.consumer.ldap.repo.user.mapper.LoginAttributesMapper;
import org.mytech.consumer.ldap.repo.user.mapper.PersonMapper;
...
public class UserldapRepositoryImpl implements LoginRepository,
	...
	final Filter f = new EqualsFilter(IConstant.MAIL, email);
	final boolean authed = ldapTemplate.authenticate(IConstant.EMPTY_STR, f.encode(),
		password);
	...
	final Optional<List<Login>> login = Optional
		.ofNullable(ldapTemplate
			.search(query().base("").where(IConstant.S_AM_ACCOUNT_NAME)
	...
	do{
	    ...
	    final EqualsFilter equalsFilter =  new EqualsFilter(IConstant.OBJECTCLASS, IConstant.PERSON);
	...
	final List<Person> ps = ldapTemplate.search(query().where(IConstant.OBJECTCLASS)
		.is(IConstant.PERSON), new PersonMapper());
